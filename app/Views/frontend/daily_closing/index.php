									...
									<form action="<?php echo base_url(); ?>/dailyclosing_online" method="post">
										<div class="row" style="margin: 10px 0px">
											...
											<div class="col-md-2" align="right">
												<button type="button" class="btn btn-info btn-lg ar_btn" data-toggle="modal" data-target="#CommissionModal">Commission</button>
											</div>
									...
									<div class="modal fade" id="CommissionModal" tabindex="-1" role="dialog" aria-labelledby="CommissionModalLabel" aria-hidden="true"> 
										<div class="modal-dialog" role="document" style="max-width: 80%;">
											<div class="modal-content" style="width: 100%;">
												<div class="modal-header">
													<h5 class="modal-title" id="CommissionModalLabel">Staff and Commission</h5>
													<button type="button" class="close" data-dismiss="modal" aria-label="Close">
														<span aria-hidden="true">&times;</span>
													</button>
												</div>
													...
													<form id="familyDetailsForm">
														<div class="row" id="inputRow">
																	...
																	<div class="form-line focused">
																		...
																		<select class="form-control" id="staff_id">
																			...
																			<?php foreach ($staff as $row) { ?>
																				<option value="<?php echo $row['id']; ?>" data-name="<?php echo $row['name']; ?>"><?php echo $row['name']; ?></option>
																			<?php } ?>
																		</select>
																	</div>
																</div>
															...
															<div class="col-sm-3">
																<div class="form-group form-float">
																	<div class="form-line focused">
																		<label class="form-label">Archanai</label>
																		<select class="form-control" id="archanai_id">
																			<option value="">--Select--</option>
																		</select>
																	</div>
																</div>
															...
															<div class="col-sm-2">
																<div class="form-group form-float">
																	<div class="form-line focused">
																		...
																		<input class="form-control" type="text" id="comm_quantity" readonly>
																	</div>
																</div>
															</div>
															...
															<div class="col-sm-3">
																<div class="form-group form-float">
																	<div class="form-line focused">
																		...
																		<input class="form-control" type="text" id="comm_amount" readonly>
																	</div>
																</div>
															</div>
															<div class="col-sm-1">
																<label class="form-label"></label>
																<button type="button" class="btn btn-success" id="addCommDetails">Add</button>
															</div>
														</div>
														<br><br>
													</form>

													<h3 style="text-align: center">Commission Details</h3>
													<table id="commissionTable" class="table">
														<thead>
															<tr>
																<th style="width: 10%; text-align: center">S.No</th>
																<th style="width: 20%; text-align: center">Staff</th>
														...
														foreach ($archanai_group_details as $archanai_detail_data) {
															if (count($archanai_detail_data['deities']) > 0) {
																...
																foreach ($archanai_detail_data['deities'] as $archanai_deity) {
																	echo '<tr><td colspan="6" style="text-align: center;"><h5>' . strtoupper($archanai_deity['deity_name']) . '</h5></td></tr>';
																	foreach ($archanai_deity['products'] as $archanai_detail) {
																		...
																		<tr>
																			...
																			<td style="padding: 5px 10px!important;">
																				<?php
																					echo $archanai_detail['paid_through'];
																					?>
																			</td>
																...
																echo '<tr><td colspan="4" style="text-align: center;"><h6>' . strtoupper($archanai_detail_data['title']) . ' Total</h4></td><td style="padding: 5px 10px!important;text-align:left;font-weight:bold;">' . $sannathi_qty . '</td><td style="padding: 5px 10px!important;text-align:right;font-weight:bold;">' . number_format($sannathi_total, 2) . '</td></tr>';
															}
														}
													}
													?>
...
<script>
	$(document).ready(function() {
		$("#staff_id").change(function() {
			...
			var date = $('#dailyclosing_start_date').val();
			$.ajax({
				url: "<?php echo base_url(); ?>/dailyclosing_online/get_archanai_products",
				method: "POST",
				data: {id: staff_id, date: date},
				datatype: "json",
				success: function(response) {
					response = JSON.parse(response);
					var archanai_dropdown = $("#archanai_id");
					archanai_dropdown.empty(); 
					archanai_dropdown.append('<option value="">Select Archanai</option>');

					if (response.length > 0) {
						$.each(response, function(index, item) {
							archanai_dropdown.append('<option value="' + item.id + '" data-name="' + item.name_eng + '">' + item.name_eng + '</option>');
						});
					}
				}
			})
		...
		$("#archanai_id").change(function() {
			var archanai_id = $('#archanai_id').val();
			var date = $('#dailyclosing_start_date').val();
			$.ajax({
				url: "<?php echo base_url(); ?>/dailyclosing_online/get_archanai_details",
				method: "POST",
				data: {id: archanai_id, date: date},
				datatype: "json",
				success: function(response) {
					response = JSON.parse(response);
					...
					if (response.length > 0) {
						var total_quantity = response[0].total_quantity;
						var total_commission = response[0].total_commission;

						$("#comm_quantity").val(total_quantity);
						$("#comm_amount").val(total_commission);
					} else {
						$("#comm_quantity").val('');
						$("#comm_amount").val('');
					}
		...
		$("#addCommDetails").click(function() {
			var staff_id = $("#staff_id").val();
			var staff_name = $("#staff_id option:selected").data('name');
			var archanai_id = $("#archanai_id").val();
			var archanai_name = $("#archanai_id option:selected").data('name');
			var comm_quantity = $("#comm_quantity").val();
			var comm_amount = $("#comm_amount").val();

			if (!staff_id || !archanai_id || !comm_quantity || !comm_amount) {
				alert("Please fill in all fields.");
				return;
			}

			var count = $("#commissionTable tbody tr").length;
			var serial_no = count + 1; 

			var html_c = '<tr id="rmv_comm_row' + count + '">';
			html_c += '<td style="width: 5%; text-align: center;">' + serial_no + '</td>'; // S.No column
			html_c += '<td style="width: 30%; text-align: center;"><input type="hidden" style="text-align: center; border: none;width: 100%;" name="commission[' + count + '][staff_id]" readonly value="' + staff_id + '">' + staff_name +'</td>';
			html_c += '<td style="width: 15%; text-align: center;"><input type="hidden" style="text-align: center; border: none;width: 100%;" name="commission[' + count + '][product_id]" readonly value="' + archanai_id + '">' + archanai_name +'</td>';
			html_c += '<td style="width: 15%; text-align: center;"><input type="hidden" name="commission[' + count + '][quantity]" value="' + comm_quantity + '">' + comm_quantity + '</td>';
			html_c += '<td style="width: 15%; text-align: center;"><input type="hidden" name="commission[' + count + '][amount]" value="' + comm_amount + '">' + comm_amount + '</td>';
			html_c += '<td style="width: 10%; text-align: center;"><a class="btn btn-danger btn-rad" onclick="removeCommissionrow(' + count + ')" style="width:auto;padding: 0px 3px !important;"><span>X</span></a></td>';
			html_c += '</tr>';

			$("#commissionTable tbody").append(html_c);
			updateSerialNumbers();

			$("#staff_id").val('');
			$("#archanai_id").empty();  // Clear all options in the dropdown
			$("#archanai_id").append('<option value="">--Select--</option>');
			$("#comm_quantity").val('');
			$("#comm_amount").val('');
		});

		function removeCommissionrow(count) {
			$("#rmv_comm_row" + count).remove();
			updateSerialNumbers(); 
		}
