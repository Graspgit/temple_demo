		...
		<tr>
			<td colspan="2" style="width: 100%;">
				<?php if (!empty($fdate) && !empty($tdate) && strtotime($fdate) && strtotime($tdate)) { ?>
					<h3 style="text-align:center;"> PRASADAM VOUCHER
						<?php echo date("d/m/Y", strtotime($fdate)) . ' - ' . date("d/m/Y", strtotime($tdate)); ?></h3>
				<?php } else { ?>
		...
		<table border="1" width="100%" align="center">
			...
			<tbody>
				...
				$builder = $db->table('prasadam')
					...
					->select('total_amount,prasadam.id, prasadam.date, prasadam.customer_name, prasadam.collection_date, prasadam.total_amount, prasadam.serve_time, prasadam.session, prasadam.payment_type, pm.name as payment_mode_name')
				...
				foreach ($dat as $row) {
					$payfors = $db->table('prasadam_booking_details')
						...
						->select('prasadam_setting.name_eng, prasadam_setting.name_tamil, prasadam_booking_details.quantity,out_rate,is_out_rate')
				...
				$payfor['name_tamil'].($payfor["is_out_rate"] == 0?"(உள்புறம்)":"(வெளிப்புறம்)"):
				    $payfor['name_tamil']). " (Quantity: " . $payfor['quantity'] . ")<br>";
					}
					?>
					<tr>
