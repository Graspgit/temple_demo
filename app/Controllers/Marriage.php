...
class Marriage extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'tot_cnts'=>$this->marriageModel->getRegistrationsWithDetailsTotCnts($from_date,$to_date,$status),
    ...
    public function store()
    {
        ...
        if (!$validation->setRules($rules)->run($this->request->getPost())) {
            echo json_encode(["status"=>false,"errMsg"=>implode(',', $validation->getErrors())]);
        ...
        if (!$availability['is_available']) {
            echo json_encode(["status"=>false,"errMsg"=>'Selected slot is no longer available']);
        }
        ...
        if ($bride_age < $min_bride_age) {
             echo json_encode(["status"=>false,"errMsg"=>"Bride must be at least {$min_bride_age} years old"]);
        }

        if ($groom_age < $min_groom_age) {
            echo json_encode(["status"=>false,"errMsg"=>"Groom must be at least {$min_groom_age} years old"]);
        }
        ...
        try {
            ...
            $registration_data['guest_count'] = intval($this->request->getPost('guest_count'));
            $registration_data['created_by'] = 0;
...
//die("R");
           ...
           // return redirect()->to('/marriage/view/' . $registration_id)->with('success', 'Marriage registration created successfully');
            ...
            echo json_encode(["status"=>false,"errMsg"=> 'Failed to create registration. Please try again.'.$e->getMessage()]);
        }
   ...
   private function generateRegistrationNumber()
    {
        ...
        $last_number = $this->marriageModel
            ...
            ->like('registration_number', $prefix . $year . $month, 'after')
            ->orderBy('id', 'DESC')
            ->first();
           
        $sequence = 1;
        if ($last_number && !empty($last_number['registration_number'])) {
