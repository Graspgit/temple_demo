...
class Prasadamsetting extends BaseController
{
	...
	public function save()
	{
		...
		$outdoor = (isset($_POST["outdoor"])?intval($_POST["outdoor"]):0);
		$out_rate = (isset($_POST["out_rate"])?intval($_POST["out_rate"]):0);
		...
		if($chk_out_rate)
		{
		    $data['out_rate'] = floatval($out_rate);
		    $data['outdoor'] = intval($outdoor);
		}
		else
		{
		    $data['out_rate'] = 0;
	...
	public function prasadam_setting_validation()
	{
		...
		if (empty($name_eng) || empty($name_tamil) || empty($amount) || empty($shortcode) || ($chk_out_rate && ($out_rate) == 0)) {
			$data['err'] = "Please Fill Required Fields";
			$data['succ'] = '';
		}
		else if (strlen($shortcode) != 4)
		{
		    $data['err'] = "Shorcode must be 4 length";
			$data['succ'] = '';
		}
		else {
